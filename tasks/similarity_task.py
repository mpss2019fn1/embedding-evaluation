...
class SimilarityTask(Task):
    ...
    def compute(self, *args, **kwargs):
        our_similarities = []
        data_similarities = []
        csv_reader = DictReader(self.csv_wikidata_results, fieldnames=self.source['columns'],
                                delimiter=self.source['delimiter'])
        assert len(self.source['columns']) == 3
        pbar = tqdm(csv_reader, total=self.size)
        with self.file_task_logger.new_file('similarities.csv').open('w') as similarity_file:
            for row in pbar:
                word1 = self.get_word_vector(row[self.source['columns'][0]])
                word2 = self.get_word_vector(row[self.source['columns'][1]])
                x = self.metric(word1, word2)
                similarity_file.write(','.join((row[self.source['columns'][0]],
                                                row[self.source['columns'][1]], str(x))) + '\n')
                if not isnan(x):
                    our_similarities.append(x)
                    data_similarities.append(row[self.source['columns'][2]])
